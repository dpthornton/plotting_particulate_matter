...
# Plot showing the total PM2.5 emission from motor vehicles in Baltmore City 
...
comb_data <- merged_data[merged_data$fips == "24510",]
comb_data <- comb_data[comb_data$EI.Sector %like% "Vehicles",]

group_by_year <- aggregate(Emissions ~ year + EI.Sector, data=comb_data, sum)

ggplot(data=group_by_year, aes(x=as.factor(year), y=Emissions, fill=factor(EI.Sector))) +
  geom_bar(stat="identity") +
  ggtitle("Motor vehicle particulate matter (PM2.5) emissions per year in Baltimore City") +
...
ggsave("plot5.png",dpi=300,device="png")